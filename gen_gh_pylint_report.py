...
def pylint_project(module_path: str, branch_url: str, errorlog: TextIOWrapper):
    ...
    for filepath in glob(glob_pattern, recursive=True):
        ...
        for r in file_results:
            ...
            def filepath_to_linktext(filepath: str) -> str:
                """short link text: filename without directory and the .py extension

                Args:
                    filepath ([str]): relative path to the file in the project

                Returns:
                    [str]: abbreviated link text for MarkDown table
                """
                try:
                ...
                return file.removesuffix(".py")  # type: ignore  # run this on >= 3.9

            linktext = filepath_to_linktext(filepath)
...
def main(module_dir: str, output_file: str, branch_url: str, errorlog_file: str):
    ...
    mylink = f"[{__file__}]({branch_url}/({__file__})"
    if summary_file:
        with open(summary_file, "a", encoding="utf-8") as fp:
            fp.write(
                f"### PyLint summary generated by {mylink} on **xcp/\\*\\*/*.py**\n"
            )
            fp.write(panda_overview.to_markdown())

            fp.write(
                f"\n### PyLint results generated by {mylink} on **xcp/\\*\\*/*.py**\n"
            )
